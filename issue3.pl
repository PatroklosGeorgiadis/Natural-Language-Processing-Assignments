...
/* June 2020						*/
/* Contributor :Patroklos Georgiadis			*/
/* July 2022						*/
/*----------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------*/
/* story in hide.txt :				*/
/* simon seeks. john hides. simon finds john.	                        */
/*----------------------------------------------------------------------*/
...
%Read story from file, analyse it and store it in the knowledge base
understand(Text):-
...
make_statements():-
  ...
  tell(Sentence), confirm_s());                      /* add entry to the KB */
...
make_questions():-
  write("Go ahead and ask me something"), nl,
  read_string(user_input, "\n", "\r\t ", _, Line),   /* get user's question */
  question_to_statement(Line, ClearText),            /* remove question marks */

  (lexical_analyse(ClearText, [Sentence]),
  ask(Sentence),confirm_q());                        /* ask question in the KB */

  (write("Try again"),nl,
  make_questions()).

confirm_s():-
  write(' Any other statements? [Yes/No]'), nl,
  read_string(user_input, "\n", "\r\t ", _, Line),
  (Line="Yes" -> make_statements();
  Line="No" -> write('Ok, bye!');
  confirm_s()).

confirm_q():-
  write(' Any other questions? [Yes/No]'), nl,
  read_string(user_input, "\n", "\r\t ", _, Line),
...
/* Intransitive Verbs (iv)				*/
/*---------------------------------------------------------------------*/
% needed for new example :
iv(iv(seeks))-->[seeks].
iv(iv(seek))-->[seek].
iv(iv(seeking))-->[seeking].
iv(iv(hides))-->[hides].
iv(iv(hide))-->[hide].
iv(iv(hiding))-->[hiding].
...
/*---------------------------------------------------------------------*/
% needed for new example :
sem_iv(seeks,s)-->[seeks].
sem_iv(seeks,q)-->[seeking].
sem_iv(hides,s)-->[hides].
sem_iv(hides,q)-->[hiding].
% extension of vocabulary :
sem_iv(runs,s)	-->[runs].
...
/* Pronoun                                                                      */
/*------------------------------------------------------------------------------*/
/* Pronoun (pronoun): Οι αντωνυμίες, χρησιμοποιούνται με τον ίδιο τρόπο που	*/
/* χρησιμοποιούνται τα Proper Nouns						*/
/*------------------------------------------------------------------------------*/
